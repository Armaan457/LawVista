...
groq_api_key = config('GROQ_API_KEY_2', default='your_groq_api_key')
...
def read_case_files(case_file_paths):
    ...
    for file_path in case_file_paths:
        try:
            response = requests.get(R2_BASE_URL + file_path)
            if response.status_code == 200:
                case_files.append(response.text[:3000])
...
def predict_case_outcome(file_contents, similar_case_paths):
    """
    ...
    # similar_case_paths = retrieve_similar_cases(file_contents)
