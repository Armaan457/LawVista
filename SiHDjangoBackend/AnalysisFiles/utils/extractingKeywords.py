...
def extract_search_terms(user_prompt):
    ...
    Pick the case info as it is, don't add unnecessary words.
    never use the word versus
    ...
    classification_result = json.loads(chat_completion.choices[0].message.content.strip())["values"]["Search_query"]
