...
class TextSummaryAPIView(APIView):
    def post(self, request):
        ...
        with ThreadPoolExecutor() as executor:
            lsi_future = executor.submit(legal_st, input_text)
            summary_future = executor.submit(summarize_input_text, input_text)

            lsi = lsi_future.result()
            summary_text, path = summary_future.result()

        outcome = predict_case_outcome(summary_text, path)
